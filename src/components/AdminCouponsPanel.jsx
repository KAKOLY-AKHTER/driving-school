...
function Pager({ page, pages, total, onChange, children }) {
  const pageNumbers = Array.from({ length: Math.min(7, pages) }, (_, index) => Math.min(Math.max(1, page - 3) + index, pages)).filter((value, index, values) => values.indexOf(value) === index)
  const buttonStyle = (active = false) => ({ minWidth: '38px', padding: '.5rem .65rem', border: `1px solid ${active ? BLUE : '#CBD5E1'}`, borderRadius: '8px', background: active ? BLUE : '#fff', color: active ? '#fff' : '#334155', fontWeight: 800, cursor: 'pointer' })
  return <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '.75rem', flexWrap: 'wrap', margin: '0 0 1rem' }}><div style={{ display: 'flex', alignItems: 'center', gap: '.65rem', flexWrap: 'wrap' }}>{children}<span style={{ color: '#475569', fontSize: '.9rem' }}>Page {page} of {pages} · {total} coupon{total === 1 ? '' : 's'}</span></div><div style={{ display: 'flex', gap: '.45rem', flexWrap: 'wrap' }}><button type="button" disabled={page <= 1} onClick={() => onChange(page - 1)} style={{ ...buttonStyle(), cursor: page <= 1 ? 'not-allowed' : 'pointer', opacity: page <= 1 ? .55 : 1 }}>Previous</button>{pageNumbers.map(number => <button type="button" key={number} aria-current={number === page ? 'page' : undefined} onClick={() => onChange(number)} style={buttonStyle(number === page)}>{number}</button>)}<button type="button" disabled={page >= pages} onClick={() => onChange(page + 1)} style={{ ...buttonStyle(), cursor: page >= pages ? 'not-allowed' : 'pointer', opacity: page >= pages ? .55 : 1 }}>Next</button></div></div>
  return <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '.75rem', flexWrap: 'wrap', margin: '0 0 1rem' }}><div style={{ display: 'flex', alignItems: 'center', gap: '.65rem', flexWrap: 'wrap' }}>{children}<span style={{ color: '#475569', fontSize: '.9rem' }}>Page {page} of {pages} · {total} coupon{total === 1 ? '' : 's'}</span></div><div style={{ display: 'flex', gap: '.45rem' }}><button type="button" disabled={page <= 1} onClick={() => onChange(page - 1)} style={{ padding: '.5rem .8rem', border: '1px solid #CBD5E1', borderRadius: '8px', background: '#fff', cursor: page <= 1 ? 'not-allowed' : 'pointer' }}>Previous</button><button type="button" disabled={page >= pages} onClick={() => onChange(page + 1)} style={{ padding: '.5rem .8rem', border: '1px solid #CBD5E1', borderRadius: '8px', background: '#fff', cursor: page >= pages ? 'not-allowed' : 'pointer' }}>Next</button></div></div>
}
