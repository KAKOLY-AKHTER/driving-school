...
import { ImportanceEducationLesson, NewDrivingLawsLesson } from '../components/online-course/ChapterOneLessons'
...
export default function OnlineEducationCoursePage() {
  ...
  const isDetailedLesson = isDriverLicenseLesson || isObeyingLawsLesson || isImportanceLesson || isNewLawsLesson
  ...
  return (
    <div className="oe-course-page">
      <style>{`
        ...
        @media(max-width:620px){.oe-importance-intro{grid-template-columns:1fr;gap:.8rem}.oe-importance-intro img{width:155px;max-height:175px}.oe-importance-intro p{font-size:.9rem}.oe-law-card{padding:.85rem .9rem}.oe-fee-table{min-width:430px}}
                    ...
                    <div className="oe-overview-link-list">
                      {chapter.topics.map((topic, lessonIndex) => (
                        <button type="button" onClick={() => selectLesson(activeChapter, lessonIndex)} key={topic}>
                          ...
                          <strong>{activeChapter === 0 && topic === 'Test 1' ? 'Chapter 1 Test' : topic}</strong>
