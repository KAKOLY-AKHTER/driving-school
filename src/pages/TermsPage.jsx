...
const SECTIONS = [
  ...
  },
  {
    ...
    body: 'A course-completion certificate is prepared only after the required course work is completed, a passing Test 11 / Final Test result is recorded in the student account, and the request is reviewed by the school. Every printed certificate receives a unique certificate number and is handed to the student by the school; certificates are not available as portal downloads. An account holder may purchase a duplicate certificate whenever needed by using the original account email and password; each paid duplicate order creates a separate request and is charged at the duplicate-certificate fee shown at checkout. Duplicate requests are reviewed before preparation and each receives its own certificate number. A duplicate certificate does not cancel or invalidate the original certificate unless we notify you otherwise. Certificate fees are non-refundable once a request has been prepared or handed over, except where required by law. We may hold, correct, or cancel a certificate record if payment is reversed, required information is inaccurate, fraud or account misuse is suspected, or issuance would violate applicable law or regulatory requirements. Please contact us promptly if you need a correction.',
  },
  {
