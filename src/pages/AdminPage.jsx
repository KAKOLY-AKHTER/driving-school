...
const legacyDisplayText = (value) => {
  const text = String(value ?? '').trim()
  return !text || text === '0' || text === '0000-00-00' ? '' : text
}
const legacyDisplayDate = (value) => {
  const text = legacyDisplayText(value)
  if (!text) return ''
  const date = new Date(text)
  return Number.isNaN(date.getTime()) ? text : date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
}
const legacyDisplayGender = (value) => {
  const text = legacyDisplayText(value).toLowerCase()
  if (text === 'f' || text === 'female') return 'Female'
  if (text === 'm' || text === 'male') return 'Male'
  if (text === 'x' || text === 'non-binary' || text === 'nonbinary') return 'Non-binary'
  return legacyDisplayText(value)
}
const legacyDisplayProgress = (value) => {
  const text = legacyDisplayText(value)
  const match = text.match(/^(\d+)\.\/([^/]+)$/)
  return match ? `Legacy course step ${match[1]} (${match[2]})` : text
}
...
export default function AdminPage() {
  ...
  const openLegacyDetails = useCallback(async (student) => {
    const candidateId = String(student?.legacyCandidateId || '').trim()
    if (!candidateId) return
    const requestId = legacyDetailsRequestRef.current + 1
    legacyDetailsRequestRef.current = requestId
    setLegacyDetails({ selected: student, records: [], credits: [], packages: [], fees: [], certificateProfiles: [], creditSearch: '', creditStatus: 'all', creditPage: 1, loading: true, error: '' })
    try {
      ...
      setLegacyDetails({ selected, records, credits: Array.isArray(response?.credits) ? response.credits : [], packages: Array.isArray(response?.packages) ? response.packages : [], fees: Array.isArray(response?.fees) ? response.fees : [], certificateProfiles: Array.isArray(response?.certificateProfiles) ? response.certificateProfiles : [], creditSearch: '', creditStatus: 'all', creditPage: 1, loading: false, error: '' })
    } catch (error) {
      if (legacyDetailsRequestRef.current !== requestId) return
      setLegacyDetails({ selected: student, records: [], credits: [], packages: [], fees: [], certificateProfiles: [], creditSearch: '', creditStatus: 'all', creditPage: 1, loading: false, error: error?.message || 'Legacy student details could not be loaded.' })
    }
  }, [])
  ...
  const saveUserEdit = async (event) => {
    ...
    setUserEdit(current => ({ ...current, saving: true, error: '' }))
    try {
      const email = userEdit.form.email.trim().toLowerCase()
      if (!/^\S+@\S+\.\S+$/.test(email)) {
        setUserEdit(current => ({ ...current, saving: false, error: 'Enter a valid email address.' }))
        return
      }
      const result = await api.adminUpdateUser(userEdit.account.uid, { ...userEdit.form, displayName, name: displayName, email })
      const updated = result?.user || { ...userEdit.account, ...userEdit.form, displayName, name: displayName, email }
      setUsers(current => current.map(account => account.uid === userEdit.account.uid ? { ...account, ...updated } : account))
      setUserEdit(null)
      setMsg('User updated.')
      setTimeout(() => setMsg(''), 2200)
    } catch (error) {
      setUserEdit(current => ({ ...current, saving: false, error: error?.message || 'User could not be updated.' }))
    }
  }
  ...
  const handleDeleteUser = (account) => {
    ...
    requestConfirmation('Delete user account?', `Remove ${name} from the site records? This action also deletes the sign-in account and cannot be undone.`, async () => {
  ...
  const saveEnrollment = async () => {
    if (!enrollmentEdit) return
    const { account, course } = enrollmentEdit
    if (!enrollmentForm.title.trim()) { setMsg('Course title is required.'); setTimeout(() => setMsg(''), 2200); return }
    try {
      const result = await api.adminUpdateUserCourse(account.uid, { enrollmentId: course.enrollmentId || '', courseId: course.id || '', enrolledAt: course.enrolledAt || '', course: enrollmentForm })
      setUsers(previous => previous.map(item => item.uid !== account.uid ? item : { ...item, courses: (item.courses || []).map(entry => (entry.enrollmentId === course.enrollmentId || (entry.id === course.id && entry.enrolledAt === course.enrolledAt)) ? { ...entry, ...(result?.course || enrollmentForm) } : entry) }))
      setEnrollmentEdit(null)
  ...
  const handleDeleteEnrollment = ({ account, course }) => requestConfirmation(
    ...
    `Remove ${adminUserName(account)}'s ${course.title || COURSE_MAP[course.id] || 'course'} enrollment?`,
    async () => {
      await api.adminDeleteUserCourse(account.uid, { enrollmentId: course.enrollmentId || '', courseId: course.id || '', enrolledAt: course.enrolledAt || '' })
      setUsers(previous => previous.map(item => item.uid !== account.uid ? item : { ...item, courses: (item.courses || []).filter(entry => !(entry.enrollmentId === course.enrollmentId || (entry.id === course.id && entry.enrolledAt === course.enrolledAt))) }))
    ...
    const loadLegacyStudents = async () => {
      ...
      try {
        ...
        setLegacyMeta({
          ...
          uniqueAccounts: Number(response?.uniqueAccounts || 0), linkedAccounts: Number(response?.linkedAccounts || 0), duplicateEmailGroups: Number(response?.duplicateEmailGroups || 0), recordsWithoutValidEmail: Number(response?.recordsWithoutValidEmail || 0),
  ...
  const legacyContactPhoneFields = legacySelectedRecord ? (() => {
    const seen = new Set()
    return [
      ['Primary phone', legacyDisplayText(legacySelectedRecord.phone)],
      ['Second phone', legacyDisplayText(legacySelectedRecord.secondaryPhone)],
      ['Alternate phone', legacyDisplayText(legacySelectedRecord.alternatePhone)],
    ].filter(([, phone]) => {
  ...
  const legacyDrivingDetails = legacySelectedRecord ? [
    ['License number', legacyDisplayText(legacySelectedRecord.licenseNumber)],
    ['License issued', legacyDisplayDate(legacySelectedRecord.licenseIssuedAt)],
    ['License expires', legacyDisplayDate(legacySelectedRecord.licenseExpiresAt)],
    ['Permit number', legacyDisplayText(legacySelectedRecord.permitNumber)],
    ['Permit issued', legacyDisplayDate(legacySelectedRecord.permitIssuedAt)],
    ['Permit expires', legacyDisplayDate(legacySelectedRecord.permitExpiresAt)],
    ['Uses lenses', legacySelectedRecord.usesLenses === true ? 'Yes' : legacySelectedRecord.usesLenses === false ? 'No' : ''],
  ] : []
  const legacyDetailSections = legacySelectedRecord ? [
    ['Student & contact', [['First name', legacyDisplayText(legacySelectedRecord.firstName)], ['Middle name', legacyDisplayText(legacySelectedRecord.middleName)], ['Last name', legacyDisplayText(legacySelectedRecord.lastName)], ['Username', legacyDisplayText(legacySelectedRecord.username)], ['Date of birth', legacyDisplayDate(legacySelectedRecord.dob)], ['Gender', legacyDisplayGender(legacySelectedRecord.gender)], ...legacyContactPhoneFields]],
  ...
  const legacyCertificateProfiles = Array.isArray(legacyDetails?.certificateProfiles) ? legacyDetails.certificateProfiles : []
  const legacyCertificateTests = legacyCertificateProfiles.flatMap(profile => Array.isArray(profile.tests) ? profile.tests : [])
  const legacyPassedCertificateTests = legacyCertificateTests.filter(test => Number.isFinite(Number(test.score)) && Number(test.score) >= 75).length
  const primaryLegacyCertificateProfile = legacyCertificateProfiles[0] || null
    ...
    <>
      <style>{`
        ...
        @media (max-width: 560px) {
          ...
          .admin-table-wrap table.legacy-mobile-cards { min-width:0 !important; }
          .admin-table-wrap .legacy-mobile-cards thead { display:none; }
          .admin-table-wrap .legacy-mobile-cards,.admin-table-wrap .legacy-mobile-cards tbody,.admin-table-wrap .legacy-mobile-cards tr,.admin-table-wrap .legacy-mobile-cards td { display:block; width:100%; }
          .admin-table-wrap .legacy-mobile-cards tbody tr { margin:.65rem; width:calc(100% - 1.3rem); border:1px solid #DCE7F3; border-radius:12px; overflow:hidden; background:#fff; }
          .admin-table-wrap .legacy-mobile-cards tbody tr:hover { background:#fff; transform:none; }
          .admin-table-wrap .legacy-mobile-cards td { display:grid; grid-template-columns:minmax(106px,.78fr) minmax(0,1.22fr); gap:.65rem; align-items:start; padding:.62rem .75rem !important; border:0 !important; border-bottom:1px solid #EEF2F7 !important; }
          .admin-table-wrap .legacy-mobile-cards td:last-child { border-bottom:0 !important; }
          .admin-table-wrap .legacy-mobile-cards td::before { content:attr(data-label); color:#64748B; font-family:var(--font-mono); font-size:.63rem; font-weight:800; letter-spacing:.055em; text-transform:uppercase; }
          .admin-table-wrap .legacy-mobile-cards td[colspan] { display:block; text-align:center; }
          .admin-table-wrap .legacy-mobile-cards td[colspan]::before { content:none; }
        }
...
Near and Long pricing is applied automatically from the selected city and verified by the server.
      ...
      {legacyDetails && (
        <div role="presentation" onClick={event => { if (event.target === event.currentTarget) setLegacyDetails(null) }} style={{ position: 'fixed', inset: 0, zIndex: 15000, display: 'grid', placeItems: 'center', padding: '1rem', background: 'rgba(10,22,40,.68)', backdropFilter: 'blur(9px)' }}>
          <section role="dialog" aria-modal="true" aria-labelledby="legacy-student-details-title" style={{ width: 'min(100%, 1180px)', maxHeight: '92vh', overflowY: 'auto', padding: '1.7rem', borderRadius: '22px', background: '#fff', boxShadow: '0 30px 90px rgba(10,22,40,.34)' }}>
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', padding: '1.2rem 1.3rem', border: '1px solid #D7E6F5', borderRadius: '16px', background: 'linear-gradient(135deg,#F8FBFF 0%,#EEF6FF 100%)' }}>
              <div style={{ display: 'flex', minWidth: 0, alignItems: 'center', gap: '.85rem' }}><div aria-hidden="true" style={{ width: '48px', height: '48px', flex: '0 0 auto', display: 'grid', placeItems: 'center', borderRadius: '14px', background: `linear-gradient(135deg,${SKY_BLUE},#0A2A5E)`, color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.25rem' }}>{String(legacySelectedRecord?.displayName || 'L').trim().charAt(0).toUpperCase()}</div><div style={{ minWidth: 0 }}><p style={{ margin: 0, color: GOLD_DEEP, fontFamily: 'var(--font-mono)', fontSize: '.7rem', fontWeight: 900, letterSpacing: '.11em', textTransform: 'uppercase' }}>Legacy student archive</p><h2 id="legacy-student-details-title" style={{ margin: '.28rem 0 0', color: DARK, fontFamily: 'var(--font-display)', fontSize: 'clamp(1.35rem,3vw,1.75rem)', lineHeight: 1.15 }}>{legacySelectedRecord?.displayName || 'Legacy student'}</h2><p style={{ margin: '.35rem 0 0', color: '#526C88', overflowWrap: 'anywhere' }}>{legacySelectedRecord?.email || 'No email'} <span aria-hidden="true">•</span> Old ID {legacySelectedRecord?.legacyCandidateId || '—'}</p></div></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '.55rem', flexShrink: 0 }}><span style={{ padding: '.32rem .58rem', borderRadius: '999px', background: legacySelectedRecord?.activationStatus === 'activated' ? '#ECFDF3' : '#FFF7ED', color: legacySelectedRecord?.activationStatus === 'activated' ? '#087443' : '#9A5B09', fontFamily: 'var(--font-mono)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.04em' }}>{legacySelectedRecord?.activationStatus === 'activated' ? 'LINKED' : 'PENDING'}</span><button autoFocus type="button" aria-label="Close legacy student details" onClick={() => setLegacyDetails(null)} style={{ width: '40px', height: '40px', border: '1px solid #BFD2E8', borderRadius: '10px', background: '#fff', color: '#334155', fontSize: '1.45rem', cursor: 'pointer' }}>&times;</button></div>
            </header>
            <p role="note" style={{ margin: '1rem 0', padding: '.78rem .95rem', borderRadius: '11px', border: '1px solid #BFDBFE', borderLeft: `4px solid ${SKY_BLUE}`, background: '#F8FBFF', color: '#334E6F', fontSize: '.85rem', lineHeight: 1.5 }}>Read-only historical data for school administration. Legacy passwords and reset tokens were never imported and are never displayed.</p>
            {legacyDetails.error && <div role="alert" style={{ marginBottom: '1rem', padding: '.8rem 1rem', border: '1px solid #FECACA', borderRadius: '10px', background: '#FEF2F2', color: '#B91C1C', fontWeight: 750 }}>{legacyDetails.error}</div>}
            {legacyDetails.loading ? <p style={{ margin: '1.5rem 0', color: SKY_BLUE, fontWeight: 800 }}>Loading every matching old record…</p> : <>
              <section aria-label="Archive overview" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(145px,1fr))', gap: '.7rem', marginBottom: '1rem' }}>
                {[[legacyDetails.records.length, 'Archived records', '#EFF6FF', '#0755AE'], [legacyCredits.length, 'Lesson activity', '#ECFDF3', '#087443'], [legacyCertificateProfiles.length, 'Certificate profiles', '#F5F3FF', '#6D28D9'], [legacyCertificateTests.length ? `${legacyPassedCertificateTests}/${legacyCertificateTests.length}` : '—', legacyCertificateTests.length ? 'Tests passed' : 'No historic tests', '#F0FDF4', '#15803D'], [legacySelectedRecord?.activationStatus === 'activated' ? 'Linked' : 'Pending', 'New account status', legacySelectedRecord?.activationStatus === 'activated' ? '#ECFDF3' : '#FFF7ED', legacySelectedRecord?.activationStatus === 'activated' ? '#087443' : '#9A5B09']].map(([value, label, background, color]) => <div key={label} style={{ minWidth: 0, padding: '.8rem .85rem', border: '1px solid #DCE7F3', borderRadius: '12px', background, boxShadow: '0 3px 10px rgba(15,63,121,.04)' }}><div style={{ color, fontFamily: 'var(--font-display)', fontSize: '1.2rem', fontWeight: 800, lineHeight: 1.1 }}>{value}</div><div style={{ marginTop: '.28rem', color: '#64748B', fontFamily: 'var(--font-mono)', fontSize: '.64rem', fontWeight: 800, letterSpacing: '.055em', textTransform: 'uppercase' }}>{label}</div></div>)}
              </section>
              <section aria-label="Related historical records" style={{ marginBottom: '1rem', padding: '1rem', borderRadius: '12px', border: '1px solid #DCE7F3', background: '#F8FBFF' }}>
                <strong style={{ color: DARK }}>{legacyDetails.records.length} original record{legacyDetails.records.length === 1 ? '' : 's'} found for this email</strong>
                <p style={{ margin: '.3rem 0 .75rem', color: '#526C88', fontSize: '.86rem' }}>Select any old record below to see its complete stored details. Each row remains separate in the archive.</p>
                <div style={{ display: 'flex', gap: '.55rem', flexWrap: 'wrap' }}>{legacyDetails.records.map(record => {
                  const selected = String(record.legacyCandidateId || '') === String(legacySelectedRecord?.legacyCandidateId || '')
                  return <button key={record.legacyCandidateId} type="button" onClick={() => setLegacyDetails(current => ({ ...current, selected: record }))} style={{ padding: '.5rem .7rem', border: `1px solid ${selected ? SKY_BLUE : '#CBD5E1'}`, borderRadius: '9px', background: selected ? '#EAF3FF' : '#fff', color: selected ? SKY_BLUE : '#334155', fontWeight: 800, cursor: 'pointer' }}>Old ID {record.legacyCandidateId || '—'}{record.legacyJoinedAt ? ` · ${legacyDisplayDate(record.legacyJoinedAt)}` : ''}</button>
                })}</div>
              </section>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(330px,1fr))', gap: '1rem' }}>
                {legacyDetailSections.map(([title, fields]) => {
                  const isEmpty = !fields.some(([, value]) => Boolean(value))
                  const canCollapse = isEmpty && ['Health information', 'Notes & references'].includes(title)
                  return <section key={title} style={{ minWidth: 0, padding: canCollapse ? '.9rem 1.15rem' : '1.15rem', borderRadius: '14px', border: '1px solid #DCE7F3', borderTop: `3px solid ${SKY_BLUE}`, background: '#fff', boxShadow: '0 4px 14px rgba(15,63,121,.045)' }}>
                    {canCollapse ? <details><summary style={{ color: '#0F3F79', fontSize: '1rem', fontWeight: 900, cursor: 'pointer' }}>{title} <span style={{ color: '#7B8CA2', fontSize: '.82rem', fontWeight: 600 }}>— no archived information</span></summary><p style={{ margin: '.8rem 0 0', color: '#64748B', fontSize: '.88rem', lineHeight: 1.5 }}>No information was recorded in the previous website for this section.</p></details> : <><h3 style={{ margin: '0 0 .85rem', color: '#0F3F79', fontSize: '1rem', fontWeight: 900 }}>{title}</h3><dl style={{ display: 'grid', gap: '.72rem', margin: 0 }}>{fields.map(([label, value]) => <div key={label} style={{ display: 'grid', gap: '.18rem', paddingBottom: '.64rem', borderBottom: '1px solid #EDF2F7' }}><dt style={{ color: '#64748B', fontFamily: 'var(--font-mono)', fontSize: '.71rem', letterSpacing: '.07em', textTransform: 'uppercase', fontWeight: 800 }}>{label}</dt><dd style={{ margin: 0, color: value ? '#1E293B' : '#7B8CA2', fontSize: '.96rem', lineHeight: 1.5, whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', fontWeight: value ? 600 : 500, fontStyle: value ? 'normal' : 'italic' }}>{value || 'Not recorded in old website'}</dd></div>)}</dl></>}</section>
                })}
              </div>
              <section aria-label="Legacy driving and certificate record" style={{ marginTop: '1rem', padding: '1.15rem', borderRadius: '14px', border: '1px solid #CFE1F5', background: 'linear-gradient(145deg,#F8FBFF,#FFFFFF)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '.75rem', flexWrap: 'wrap', marginBottom: '.9rem' }}><div><h3 style={{ margin: 0, color: '#0F3F79', fontSize: '1.05rem', fontWeight: 900 }}>Driving &amp; certificate record</h3><p style={{ margin: '.2rem 0 0', color: '#526C88', fontSize: '.84rem' }}>License, permit, online-course, and certificate data retained from the previous website.</p></div><span style={{ padding: '.28rem .55rem', borderRadius: '999px', background: '#EAF3FF', color: SKY_BLUE, fontFamily: 'var(--font-mono)', fontSize: '.67rem', fontWeight: 900 }}>READ-ONLY ARCHIVE</span></div>
                {primaryLegacyCertificateProfile ? <section aria-label="Certificate archive snapshot" style={{ marginBottom: '1rem', padding: '.95rem', border: '1px solid #DDD6FE', borderRadius: '12px', background: 'linear-gradient(145deg,#FFFFFF,#FAFAFF)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '.65rem', flexWrap: 'wrap', marginBottom: '.75rem' }}><div><h4 style={{ margin: 0, color: '#5B21B6', fontSize: '.98rem', fontWeight: 900 }}>Certificate &amp; online-course snapshot</h4><p style={{ margin: '.2rem 0 0', color: '#6B7280', fontSize: '.82rem' }}>Matched only by this student’s exact same email address.</p></div><span style={{ padding: '.26rem .52rem', borderRadius: '999px', background: '#F5F3FF', color: '#6D28D9', fontFamily: 'var(--font-mono)', fontSize: '.65rem', fontWeight: 900 }}>OLD ONLINE ID {primaryLegacyCertificateProfile.legacyCandidateId || '—'}</span></div>
                  <dl style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(175px,1fr))', gap: '.65rem', margin: 0 }}>{[
                    ['Student name', primaryLegacyCertificateProfile.displayName], ['Email', primaryLegacyCertificateProfile.email], ['Phone', primaryLegacyCertificateProfile.phone], ['Date of birth', legacyDisplayDate(primaryLegacyCertificateProfile.dob)],
                    ['Address', [primaryLegacyCertificateProfile.address, primaryLegacyCertificateProfile.address2, primaryLegacyCertificateProfile.city, primaryLegacyCertificateProfile.state, primaryLegacyCertificateProfile.zipCode].filter(Boolean).join(', ')], ['Course', primaryLegacyCertificateProfile.courseName], ['Course completed', primaryLegacyCertificateProfile.courseCompleted ? 'Yes' : 'No'], ['Certificate issued', primaryLegacyCertificateProfile.certificateIssued ? 'Yes' : 'No'],
                    ['Test results', primaryLegacyCertificateProfile.tests?.length ? `${primaryLegacyCertificateProfile.tests.length} recorded · ${primaryLegacyCertificateProfile.tests.filter(test => Number(test.score) >= 75).length} passed` : 'No historic test result recorded'], ['Course progress', legacyDisplayProgress(primaryLegacyCertificateProfile.courseProgress)],
                  ].map(([label, value]) => <div key={label} style={{ minWidth: 0, padding: '.65rem .72rem', borderRadius: '9px', background: '#fff', border: '1px solid #EEEAFE' }}><dt style={{ color: '#64748B', fontFamily: 'var(--font-mono)', fontSize: '.64rem', letterSpacing: '.055em', textTransform: 'uppercase', fontWeight: 800 }}>{label}</dt><dd style={{ margin: '.28rem 0 0', color: value ? '#1E293B' : '#7B8CA2', fontSize: '.9rem', lineHeight: 1.38, overflowWrap: 'anywhere', fontWeight: value ? 650 : 500, fontStyle: value ? 'normal' : 'italic' }}>{value || 'Not recorded'}</dd></div>)}</dl>
                </section> : <p role="note" style={{ margin: '0 0 1rem', padding: '.75rem .85rem', border: '1px solid #DCE7F3', borderRadius: '10px', background: '#fff', color: '#64748B', fontSize: '.84rem' }}>No certificate or online-course profile matched this student’s exact email address.</p>}
                <h4 style={{ margin: '0 0 .65rem', color: '#0F3F79', fontSize: '.94rem', fontWeight: 900 }}>License &amp; permit record</h4>
                <dl style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(175px,1fr))', gap: '.7rem', margin: 0 }}>{legacyDrivingDetails.map(([label, value]) => <div key={label} style={{ minWidth: 0, minHeight: '92px', padding: '.75rem .8rem', border: '1px solid #DCE7F3', borderRadius: '11px', background: '#fff' }}><dt style={{ color: '#64748B', fontFamily: 'var(--font-mono)', fontSize: '.67rem', letterSpacing: '.065em', textTransform: 'uppercase', fontWeight: 800 }}>{label}</dt><dd style={{ margin: '.4rem 0 0', color: value ? '#1E293B' : '#7B8CA2', fontSize: '.95rem', lineHeight: 1.35, overflowWrap: 'anywhere', fontWeight: value ? 750 : 500, fontStyle: value ? 'normal' : 'italic' }}>{value || 'Not recorded'}</dd></div>)}</dl>
                <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #DCE7F3' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '.65rem', flexWrap: 'wrap', marginBottom: '.75rem' }}><div><h4 style={{ margin: 0, color: '#0F3F79', fontSize: '.98rem', fontWeight: 900 }}>Online course &amp; certificate record</h4><p style={{ margin: '.2rem 0 0', color: '#526C88', fontSize: '.84rem' }}>Certificate profile and all chapter results for this exact same email address.</p></div><span style={{ padding: '.28rem .55rem', borderRadius: '999px', background: '#F5F3FF', color: '#6D28D9', fontFamily: 'var(--font-mono)', fontSize: '.67rem', fontWeight: 900 }}>{legacyCertificateProfiles.length} PROFILE{legacyCertificateProfiles.length === 1 ? '' : 'S'}</span></div>
                  {legacyCertificateProfiles.length ? <div style={{ display: 'grid', gap: '.85rem' }}>{legacyCertificateProfiles.map(profile => {
                    const profileFields = [
                      ['Name', profile.displayName], ['Email', profile.email], ['Username', profile.username], ['Phone', profile.phone], ['Date of birth', legacyDisplayDate(profile.dob)], ['Joined', legacyDisplayDate(profile.joinedAt)],
                      ['Address', [profile.address, profile.address2, profile.city, profile.state, profile.zipCode].filter(Boolean).join(', ')], ['Old online ID', profile.legacyCandidateId], ['Course', profile.courseName], ['Course price', profile.coursePrice ? `$${profile.coursePrice}` : ''], ['Course completed', profile.courseCompleted ? 'Yes' : 'No'], ['Certificate issued', profile.certificateIssued ? 'Yes' : 'No'], ['Certificate details', profile.certificateDetails], ['Course progress', legacyDisplayProgress(profile.courseProgress)],
                    ]
                    const tests = Array.isArray(profile.tests) ? profile.tests : []
                    return <section key={profile.legacyCandidateId} style={{ padding: '1rem', border: '1px solid #D9D6FE', borderRadius: '12px', background: '#fff' }}><strong style={{ color: '#5B21B6' }}>Certificate profile · Old online ID {profile.legacyCandidateId || '—'}</strong><dl style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '.65rem', margin: '.75rem 0 0' }}>{profileFields.map(([label, value]) => <div key={label} style={{ minWidth: 0, padding: '.62rem .7rem', borderRadius: '9px', background: '#FAFAFF' }}><dt style={{ color: '#64748B', fontFamily: 'var(--font-mono)', fontSize: '.65rem', letterSpacing: '.055em', textTransform: 'uppercase', fontWeight: 800 }}>{label}</dt><dd style={{ margin: '.28rem 0 0', color: value ? '#1E293B' : '#7B8CA2', fontSize: '.88rem', lineHeight: 1.35, overflowWrap: 'anywhere', fontWeight: value ? 650 : 500, fontStyle: value ? 'normal' : 'italic' }}>{value || 'Not recorded'}</dd></div>)}</dl><div style={{ marginTop: '.9rem' }}><strong style={{ color: '#0F3F79', fontSize: '.9rem' }}>Chapter &amp; final-test results ({tests.length})</strong><div className="admin-table-wrap" style={{ marginTop: '.5rem' }}><table style={{ width: '100%', minWidth: '620px', borderCollapse: 'collapse' }}><thead><tr><th scope="col" style={thStyle}>Chapter / Test</th><th scope="col" style={thStyle}>Date</th><th scope="col" style={thStyle}>Score</th><th scope="col" style={thStyle}>Result</th></tr></thead><tbody>{tests.map(test => { const score = Number(test.score); const hasScore = Number.isFinite(score); const passed = hasScore && score >= 75; return <tr key={test.legacyTestId}><td style={tdStyle}>{test.testTitle}</td><td style={tdStyle}>{legacyDisplayDate(test.testDate)}</td><td style={tdStyle}>{hasScore ? `${score.toFixed(2)}%` : 'Not recorded'}</td><td style={tdStyle}><span style={{ padding: '.25rem .52rem', borderRadius: '999px', background: passed ? '#ECFDF3' : '#FEF2F2', color: passed ? '#087443' : '#B91C1C', fontFamily: 'var(--font-mono)', fontSize: '.68rem', fontWeight: 900 }}>{passed ? 'Passed' : 'Below 75%'}</span></td></tr> })}{!tests.length && <tr><td colSpan={4} style={{ ...tdStyle, textAlign: 'center', color: '#64748B' }}>No old test result was recorded for this certificate profile.</td></tr>}</tbody></table></div></div></section>
                  })}</div> : <p style={{ margin: 0, color: '#64748B', fontSize: '.88rem' }}>No online-course or certificate profile with this exact email was found. No data has been guessed or joined.</p>}
                </div>
              </section>
              <section style={{ marginTop: '1rem', padding: '1.15rem', borderRadius: '14px', border: '1px solid #DCE7F3', background: 'linear-gradient(145deg,#FFFFFF,#F8FBFF)' }}>
                ...
                <div className="admin-table-wrap"><table className="legacy-mobile-cards" style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <section style={{ marginTop: '1rem', padding: '1.15rem', borderRadius: '14px', border: '1px solid #DCE7F3', background: 'linear-gradient(145deg,#FFFFFF,#F8FBFF)' }}>
                ...
                <div className="admin-table-wrap"><table className="legacy-mobile-cards" style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>{legacyFees.map(item => <tr key={item.legacyCandidateFeeId}>
                    <td data-label="Old fee ID" style={{ ...tdStyle, fontFamily: 'var(--font-mono)', fontSize: '.82rem' }}>{item.legacyCandidateFeeId || '—'}</td>
                    <td data-label="Old student ID" style={tdStyle}>ID {item.legacyCandidateId || '—'}</td>
                    <td data-label="Fee" style={tdStyle}>{item.fee ? `$${item.fee}` : 'Not recorded'}</td>
                    <td data-label="Payment status" style={tdStyle}>{item.paymentStatus || 'Not recorded'}</td>
                    <td data-label="Recorded" style={tdStyle}>{formatDateDMY(item.insertedAt)}</td>
              ...
              <section style={{ marginTop: '1rem', padding: '1.15rem', borderRadius: '14px', border: '1px solid #DCE7F3', background: 'linear-gradient(145deg,#FFFFFF,#F8FBFF)' }}>
                ...
                <div className="admin-table-wrap"><table className="legacy-mobile-cards" style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>{visibleLegacyCredits.map(credit => <tr key={`${credit.archiveType || credit.source || 'credit'}-${credit.legacyCandidateCreditId}`}>
                    <td data-label="Old credit ID" style={{ ...tdStyle, fontFamily: 'var(--font-mono)', fontSize: '.82rem' }}>{credit.legacyCandidateCreditId || '—'}</td>
                    <td data-label="Old student ID" style={tdStyle}>ID {credit.legacyCandidateId || '—'}</td>
                    <td data-label="Schedule" style={tdStyle}><strong>{formatDateDMY(credit.scheduledDate)}</strong><p style={{ margin: '.18rem 0 0', color: '#64748B', fontSize: '.84rem' }}>{credit.fromTime || 'Time unavailable'} – {credit.toTime || 'Time unavailable'}</p></td>
                    <td data-label="Instructor / location" style={tdStyle}><div>{credit.instructorName || (credit.legacyInstructorId ? `Instructor #${credit.legacyInstructorId}` : 'Not recorded')}</div><p style={{ margin: '.18rem 0 0', color: '#64748B', fontSize: '.84rem' }}>{credit.locationName || (credit.locationId ? `Location ${credit.locationId}` : 'Location unavailable')}</p></td>
        ...
        <div role="presentation" onClick={event => { if (event.target === event.currentTarget) setLegacyCertificateDetails(null) }} style={{ position: 'fixed', inset: 0, zIndex: 15000, display: 'grid', placeItems: 'center', padding: '1rem', background: 'rgba(10,22,40,.68)', backdropFilter: 'blur(9px)' }}>
          <section role="dialog" aria-modal="true" aria-labelledby="legacy-certificate-details-title" style={{ width: 'min(100%, 1060px)', maxHeight: '90vh', overflowY: 'auto', padding: '1.8rem', borderRadius: '20px', background: '#fff', boxShadow: '0 30px 90px rgba(10,22,40,.34)' }}>
            ...
            {legacyCertificateDetails.loading ? <p style={{ margin: '1.5rem 0', color: SKY_BLUE, fontWeight: 800 }}>Loading legacy certificate details…</p> : legacyCertificateDetails.data && <>
              ...
              {(() => {
                const drivingRecords = Array.isArray(legacyCertificateDetails.data.drivingRecords) ? legacyCertificateDetails.data.drivingRecords : []
                return <section style={{ marginTop: '1rem', padding: '1.15rem', borderRadius: '14px', border: '1px solid #CFE1F5', background: 'linear-gradient(145deg,#F8FBFF,#FFFFFF)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '.75rem', flexWrap: 'wrap', marginBottom: '.85rem' }}><div><h3 style={{ margin: 0, color: '#0F3F79', fontSize: '1.05rem', fontWeight: 900 }}>Driving-school record</h3><p style={{ margin: '.25rem 0 0', color: '#526C88', fontSize: '.86rem' }}>Name, email, contact, and driving details matched by the exact same legacy email address.</p></div><span style={{ padding: '.3rem .56rem', borderRadius: '999px', background: '#EAF3FF', color: SKY_BLUE, fontFamily: 'var(--font-mono)', fontSize: '.67rem', fontWeight: 900 }}>{drivingRecords.length} MATCH{drivingRecords.length === 1 ? '' : 'ES'}</span></div>
                  {drivingRecords.length ? <div style={{ display: 'grid', gap: '.8rem' }}>{drivingRecords.map(record => {
                    const fields = [
                      ['Name', record.displayName], ['Email', record.email], ['Phone', record.phone], ['Second / alternate phone', record.secondaryPhone || record.alternatePhone],
                      ['Address', [record.address, record.city, record.state, record.zipCode].filter(Boolean).join(', ')], ['Old driving ID', record.legacyCandidateId], ['Joined', legacyDisplayDate(record.legacyJoinedAt)],
                      ['License number', record.licenseNumber], ['License issued', legacyDisplayDate(record.licenseIssuedAt)], ['License expires', legacyDisplayDate(record.licenseExpiresAt)],
                      ['Permit number', record.permitNumber], ['Permit issued', legacyDisplayDate(record.permitIssuedAt)], ['Permit expires', legacyDisplayDate(record.permitExpiresAt)], ['Uses lenses', record.usesLenses === true ? 'Yes' : record.usesLenses === false ? 'No' : ''],
                    ]
                    return <section key={record.legacyCandidateId} style={{ padding: '1rem', border: '1px solid #DCE7F3', borderRadius: '12px', background: '#fff' }}><div style={{ marginBottom: '.75rem', color: '#0F3F79', fontWeight: 900 }}>Driving profile · Old ID {record.legacyCandidateId || '—'}</div><dl style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '.7rem', margin: 0 }}>{fields.map(([label, value]) => <div key={label} style={{ minWidth: 0, padding: '.65rem .7rem', borderRadius: '9px', background: '#F8FBFF' }}><dt style={{ color: '#64748B', fontFamily: 'var(--font-mono)', fontSize: '.66rem', fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase' }}>{label}</dt><dd style={{ margin: '.3rem 0 0', color: value ? '#1E293B' : '#7B8CA2', fontSize: '.9rem', lineHeight: 1.4, overflowWrap: 'anywhere', fontWeight: value ? 650 : 500, fontStyle: value ? 'normal' : 'italic' }}>{value || 'Not recorded'}</dd></div>)}</dl></section>
                  })}</div> : <p style={{ margin: 0, color: '#64748B', fontSize: '.9rem' }}>No driving-school profile with this exact legacy email was found. No record has been guessed or attached.</p>}
                </section>
              })()}
              <section style={{ marginTop: '1rem', padding: '1.15rem', borderRadius: '14px', border: '1px solid #DCE7F3', background: 'linear-gradient(145deg,#FFFFFF,#F8FBFF)' }}>
