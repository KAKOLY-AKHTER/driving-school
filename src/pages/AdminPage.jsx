        ...
        <div role="presentation" onClick={event => { if (event.target === event.currentTarget) setLegacyDetails(null) }} style={{ position: 'fixed', inset: 0, zIndex: 15000, display: 'grid', placeItems: 'center', padding: '1rem', background: 'rgba(10,22,40,.68)', backdropFilter: 'blur(9px)' }}>
          <section role="dialog" aria-modal="true" aria-labelledby="legacy-student-details-title" style={{ width: 'min(100%, 1180px)', maxHeight: '92vh', overflowY: 'auto', padding: '1.7rem', borderRadius: '22px', background: '#fff', boxShadow: '0 30px 90px rgba(10,22,40,.34)' }}>
            ...
            {legacyDetails.loading ? <p style={{ margin: '1.5rem 0', color: SKY_BLUE, fontWeight: 800 }}>Loading every matching old record…</p> : <>
              ...
              <section aria-label="Legacy driving record" style={{ marginTop: '1rem', padding: '1.15rem', borderRadius: '14px', border: '1px solid #CFE1F5', background: 'linear-gradient(145deg,#F8FBFF,#FFFFFF)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '.75rem', flexWrap: 'wrap', marginBottom: '.9rem' }}><div><h3 style={{ margin: 0, color: '#0F3F79', fontSize: '1.05rem', fontWeight: 900 }}>Driving record</h3><p style={{ margin: '.2rem 0 0', color: '#526C88', fontSize: '.84rem' }}>License and permit values retained from the previous website.</p></div><span style={{ padding: '.28rem .55rem', borderRadius: '999px', background: '#EAF3FF', color: SKY_BLUE, fontFamily: 'var(--font-mono)', fontSize: '.67rem', fontWeight: 900 }}>READ-ONLY ARCHIVE</span></div>
