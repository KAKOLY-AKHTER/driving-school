/**
 * Imports the old tbl_candidate_credits_cancel archive separately from normal
 * credit records. The old tables reuse credit IDs, so this never overwrites
 * a normal historic credit row.
 */
...
function toCancelledLegacyCredit(row) {
  return {
    ...
    cancelledByAdmin: yes(row.by_admin),
    removalNote: cleanText(row.removal_note, 2000),
    cancelled: true,
    source: 'aprecisi_SchDb.tbl_candidate_credits_cancel',
  }
}

if (!sourceFile) {
  console.error('Usage: node server/scripts/importLegacyCancelledStudentCredits.mjs --file "C:\\path\\old-export.json" [--apply]')
  process.exitCode = 1
} else {
  const exportData = JSON.parse(await fs.readFile(path.resolve(sourceFile), 'utf8'))
  const table = Array.isArray(exportData) ? exportData.find(item => item?.type === 'table' && item?.name === 'tbl_candidate_credits_cancel') : null
  const rows = Array.isArray(table?.data) ? table.data : []
  if (!rows.length) throw new Error('tbl_candidate_credits_cancel was not found or has no rows.')
  const credits = rows.map(toCancelledLegacyCredit).filter(credit => credit.legacyCandidateCreditId && credit.legacyCandidateId)
  ...
  if (!shouldApply) {
    ...
    try {
      ...
      const collection = client.db('driving_school').collection('legacy_student_cancelled_credits')
      await collection.createIndex({ legacyCandidateCreditId: 1 }, { unique: true, name: 'unique_legacy_student_cancelled_credit' })
      ...
      console.log('Import complete. Cancelled historic credits are retained for administrator review only.')
    } finally {
      await client.close()
    }
  }
